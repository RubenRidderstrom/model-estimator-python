...
def test_fulltest2(tmpdir):
        #   Create directory paths
    CURRENT_DIR = os.path.dirname(__file__)
    TEST_FILES_PATH = os.path.join(CURRENT_DIR, 'test_files\\full_test\\')

    #   Create sequence file path
    SEQUENCE_FILE_NAME = "testcase_20seqs_1000long_50pam.fa"
    FILE_PATH = os.path.join(TEST_FILES_PATH, SEQUENCE_FILE_NAME)

    #   Load reference Q and EQ
    REFERENCE_Q_PATH = os.path.join(TEST_FILES_PATH, 'testcase_20seqs_1000long_50pam_Q.npy')
    REFERENCE_Q = np.load(REFERENCE_Q_PATH)
    REFERENCE_EQ_PATH = os.path.join(TEST_FILES_PATH, 'testcase_20seqs_1000long_50pam_EQ.npy')
    REFERENCE_EQ = np.load(REFERENCE_EQ_PATH)
